...
export class RankEvaluator extends Evaluator {
  evaluate(recommender: Recommender, validate: boolean = false): Result {
    getRenderer().updated("Starting evaluation..");
    if (validate === false && this.cache[recommender.hash()]) {
      getRenderer().updated("USING CACHE!");
      return this.cache[recommender.hash()];
    }

    const testInteractions: Matrix<any> = validate
      ? this.problemInstance.validateInteractions
      : this.problemInstance.testInteractions;
    const keys = testInteractions.getFromRefs();
    const recommendations = keys
      .map((fromId, idx) => {
        if (idx % 10 === 0) {
          getRenderer().updated(
            `Evaluating ${idx} out of ${keys.length}`,
            true,
          );
        }
        ...
        return {
          ...
          precision: nrRecommendations
            ? found / Math.min(nrRecommendations, total)
            : 0,
        };
      });

    const averageRecall = recommendations.reduce(sumBy((it) => it.recall), 0) /
      recommendations.length;
    const averagePrecision =
      recommendations.reduce(sumBy((it) => it.precision), 0) /
      recommendations.length;

    const fScore = 2 * (averagePrecision * averageRecall) /
