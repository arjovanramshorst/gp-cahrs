...
class Renderer {
  ...
  public setHeadless(
    ...
    if (worker != undefined) {
      this.prefix = `[WORKER-${worker}] GEN-${generation} ID-${idx} | `;
    }
  }
  ...
  public updated(status?: string, onlyHeadless: boolean = false) {
    if (this.headless) {
      if (status) {
        console.log(`${green(this.prefix)}${status}`);
      }
    } else if (!onlyHeadless) {
      if (status) {
  ...
  private renderProgress() {
    if (this.progress && this.maxProgress) {
      const loadingBars = Math.floor(
        (this.progress * LOADING_BAR_LENGTH) / this.maxProgress,
      );
      const bar = [...Array(LOADING_BAR_LENGTH).keys()].map((i) =>
        i < loadingBars ? "-" : " "
      ).join("");
  ...
  private renderHeader() {
    ...
    console.log(
      red("  \\____/_/   \\_\\_| |_|_| \\_\\____/       \\____|_|    "),
    );
