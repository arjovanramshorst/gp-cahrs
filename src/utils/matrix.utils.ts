...
export abstract class Matrix<T> {
  ...
  static combine<T, K = T>(
    input: Matrix<T>[],
    func: (input: T[]) => K,
    fromRef?: EntityId,
  ) {
    const fromRefs = fromRef
      ? [fromRef]
      : [...new Set([...input.flatMap((it) => it.getFromRefs())])];
    ...
    for (let i = 0; i < fromRefs.length; i++) {
      for (let j = 0; j < toRefs.length; j++) {
        const res = input.map((it) => it.get(fromRefs[i], toRefs[j])).filter(
          isDefined,
        ) as T[];
...
export class CombinedMatrix<T extends any, K = T> extends Matrix<K> {
  ...
  constructor(
    ...
    super();
    this.fromRefs = [
      ...new Set([...matrixes.flatMap((it) => it.getFromRefs())]),
    ];
  ...
  get(fromRef: EntityId, toRef: EntityId): K | undefined {
    return this.combineFunc(
      this.matrixes.map((it) => it.get(fromRef, toRef) ?? this.nullValue),
    );
  }
...
export class DenseMatrix<T extends any> extends Matrix<T> {
  ...
  public get = (fromRef: EntityId, toRef: EntityId) => {
    return this
      .scores[this.fromRefsToIndex[fromRef]][this.toRefsToIndex[toRef]];
  };

  public set = (fromRef: EntityId, toRef: EntityId, value: T) => {
    this.scores[this.fromRefsToIndex[fromRef]][this.toRefsToIndex[toRef]] =
      value;
  };
...
export class SparseMatrix<T extends any> extends Matrix<T> {
  constructor(
    ...
    private readonly invertedMatrix: Record<
      EntityId,
  ...
  static fromToVector<T>(
    fromRef: EntityId,
    toVector: Record<EntityId, T>,
  ): SparseMatrix<T> {
