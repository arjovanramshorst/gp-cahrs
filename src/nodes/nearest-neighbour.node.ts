...
export class NearestNeighbourConfig
  extends NodeConfig<NearestNeighbourProcessor> {
  configType = `NN-node (${
    this.config.inverted ? this.config.toEntityType : this.config.fromEntityType
    ...
    return [
      ...
      ...(this.config.inverted
        ? PropertyNodeConfig.PotentialConfigs(
          problemInstance.entityMap[this.config.toEntityType],
          problemInstance.entityMap[this.config.toEntityType],
        )
        : PropertyNodeConfig.PotentialConfigs(
          problemInstance.entityMap[this.config.fromEntityType],
          problemInstance.entityMap[this.config.fromEntityType],
        )),
    ];
  }
...
export class NearestNeighbourProcessor
  ...
  prepare(instance: ProblemInstance): void {
    const matrix =
      instance.interactionMap[this.config.interactionType].interactionMatrix;
  ...
  processUser(
    ...
    if (input[0].fromEntityType !== input[0].toEntityType) {
      throw new Error(
        "NN only works on input with fromEntityType == toEntityType",
      );
    }
    ...
    const similarRefs = this.getMostSimilar(
      params.entityId,
      input[0].matrix.getRow(params.entityId),
    );
  ...
  processInverted(
    ...
    const positiveToRefs = toRefs.filter((toRef) =>
      ownerInteractions[toRef] > 0
    );
    ...
    positiveToRefs.forEach((positiveRef) => {
      const mostSimilar = this.getMostSimilar(
        positiveRef,
        input[0].matrix.getRow(positiveRef),
      );
      const score = this.interactionMatrix.get(
        params.entityId,
        positiveRef,
      ) as number;
