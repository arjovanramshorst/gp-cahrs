...
export class CombineNodeConfig extends NodeConfig<CombineNodeProcessor> {
    ...
    protected processorFactory() {
        return new CombineNodeProcessor(this.config as WithGenerated<ConfigInterface>)
    }
}

export class CombineNodeProcessor extends NodeProcessor<WithGenerated<ConfigInterface>> {
    getCombineFunc = (): (arr: number[]) => number => {
        switch(this.config.generated.strategy) {
            case "avg":
                return (arr: number[]) => arr.reduce((agg, curr) => agg + curr, 0) / arr.length
            case "max":
    ...
    process(input: SimilarityScores[], params: ProcessParams): SimilarityScores {
        ...
        return {
            ...
            matrix: new CombinedMatrix(input.map(it => it.matrix), this.getCombineFunc(), 0)       }
    }
}