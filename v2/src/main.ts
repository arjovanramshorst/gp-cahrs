 ...
 * Main loop:
 *
 * TODO: Add CF based baseline evaluation to movielens and sobazaar
 *************************************************************************
 *************************************************************************
 *************************************************************************
 */
const main = async (readProblem = CONFIG.PROBLEM.read) => {
  ...
  for (let gen = 0; gen < CONFIG.GENERATIONS; gen++) {
    ...
    const bestForGenerationFitness = await evaluateBestOfGeneration(bestForGeneration.config)
    const str = produceCsvLine(`${gen}`, "best", "-", bestForGenerationFitness, bestForGenerationFitness, bestForGeneration.config)
    appendFile(filename, str)
    console.log(`Evaluating on verification sample - DONE (${bestForGenerationFitness.performance})`)
...
const evaluateGeneration = (gen: number, configs: ConfigTree[], problem): EvaluatedConfig[] => {
  ...
  const str = produceCsvLine(`${gen}`, "gen_baseline", "-", baselineFitness, {
    fScore: 0,
    precision: 0,
    recall: 0,
    performance: 0
  ...
  return configs.map((config, idx) => {
    ...
    const str = produceCsvLine(`${gen}`, "individual", `${idx}`, fitness.raw, fitness.normalized, config);
...
const evaluateBaseline = async () => {
  ...
  const problem = await CONFIG.PROBLEM.read(CONFIG.INTERLEAVE_SIZE, CONFIG.VERIFICATION_SEED)
  const baselineFitness = fitnessScore(calcRecursive(problem.baseline, problem), problem).raw
  const str = produceCsvLine(`-`, "baseline", "-", baselineFitness, baselineFitness, problem.baseline)
  appendFile(filename, str)
  console.log(`Evaluating on verification sample - DONE (${baselineFitness.performance})`)
...
const evaluateBestOfGeneration = async (config: ConfigTree) => {
  ...
  const validationFitness = fitnessScore(calcRecursive(config, problem), problem).raw
